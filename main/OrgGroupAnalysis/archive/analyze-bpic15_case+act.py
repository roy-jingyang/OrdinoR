...
dirout = sys.argv[2]
fnout_resource_profiles = join(dirout, 'case+act.profiles.csv')
fnout_group_profiles = join(dirout, 'case+act.group-profiles.csv')
fnout_time_results = join(dirout, 'case+act.time-avg.csv')
fnout_time_results1 = join(dirout, 'case+act.time-full-info.csv')
...
if __name__ == '__main__':
    ...
    if selected_case_class == '':
        raise ValueError('A case class must be selected.')
    else:
        log = log.loc[log['(case) last_phase'] == selected_case_class]

    # NOTE: calculate activity classes, "phase", to be scoped on
    # NOTE: this should be done based on the original log data
    # NOTE: filter infrequent subprocesses (< 10%)
    freq_subprocesses = filter_events_by_frequency(
        el, 
        'subprocess', 0.1, query_only=True)
    # NOTE: filter infrequent activity classes, "phase" (< 10%)
    freq_phases = filter_events_by_frequency(
        el.loc[el['subprocess'].isin(freq_subprocesses)], 
        'phase', 0.1, query_only=True)
    # -------------------------
    ...
    # scope on specific activity classes ("phases")
    log = log.loc[log['phase'].isin(freq_phases)]
