...
class ODTSAMiner(BaseMiner):
    ...
    def fit_decision_tree(self):
        ...
        while self._temp > 0:
            ...
            if self._decide_stopping(target):
                print('Change in the target value ({:.6f}) is too insignificant.'.format(
                    target
                ))
                # exit search
                break
            else:
                # decide whether to move on with the derived split
                prob = self._accept_prob(target, self.val_target)  
    ...
    def _func_target(self, delta_dis, old_dis, delta_imp, old_imp):
        if (old_dis + delta_dis) == 0 or (old_imp + delta_imp) == 0:
            return np.inf
    ...
    def _decide_stopping(self, target):
        #return np.abs(target - self.val_target) < self.eps
        return False
    
    def _find_attr_split(self):
        # a candidate to evaluate is identified by (attr, split_rules)

        # sample an attribute uniformly
        row = np.random.choice(self.cand_attr_pool)
        result = dict()
        ...
        # sample a block regarding attribute uniformly
        idx_par = np.random.choice(range(len(l_idx_sublog)))
        par = l_idx_sublog[idx_par]
        if len(par) > 0:
            # determine possible further splits
            if attr_type == 'numeric':
                # use histogram for numeric attribute
                cand_rules = NumericRuleGenerator.HistogramSplit(
                ...
                cand_rules = CategoricalRuleGenerator.RandomTwoSubsetPartition(
                    ...
                    n_sample=1, from_population=False
                )
                cand_rules = list(cand_rules)

            if len(cand_rules) > 0:
                # evalute the derived split rules by applying them
                dis, imp = self._evaluate_split(cand_rules, attr_dim)

                # record results related to the derived rules
                result['attr'] = attr
                result['attr_dim'] = attr_dim
                result['split_rules'] = cand_rules
                # 
